...
describe RuboCop::CLI, :isolated_environment do
  ...
  describe 'option' do
    ...
    describe '--auto-gen-config' do
      ...
      it 'can merge Exclude properties', :focus do
        create_file('example1.rb', ['# encoding: utf-8',
                                    '$x = 0'])
        create_file('example2.rb', ['# encoding: utf-8',
                                    '$y = 0'])
        local_config = ['inherit_from: .rubocop_todo.yml',
                        '',
                        'Style/GlobalVars:',
                        '  Exclude:',
                        "    - 'example1.rb'"]
        create_file('.rubocop.yml', local_config)
        expect(cli.run(['--auto-gen-config'])).to eq(1)
        expected =
          ['# This configuration was generated by',
           '# `rubocop --auto-gen-config`',
           ...
           "    - 'example2.rb'"]
        actual = IO.read('.rubocop_todo.yml').split($RS)
        expected.each_with_index { |line, i| expect(actual[i]).to match(line) }

        $stdout = StringIO.new
        cli.run(%w(--format files))
        expect($stdout.string.chomp).to eq(abs('example2.rb'))
      end
